...
class DeformableCrossAttention(BaseModule):
    ...
    def forward(self,
        ...
        # print(torch.mean(confmap_kv))
        # confmap_kv = confmap_kv.reshape(bs, num_value_pts, self.num_heads, -1)
        # print(confmap_kv)
        value1 = torch.cat([value_img * confmap_kv, value_pts * (1 - confmap_kv)], dim=1)
        # value1 = torch.cat([value_img * 0.5, value_pts * (1 - 0.5)], dim=1)
        ...
        output1 = MultiScaleDeformableAttnFunction_fp32.apply(
            value1, spatial_shapes, level_start_index, sampling_locations,
            attention_weights, self.im2col_step)
        outt = output + output1
        # outt = self.conv(outt).squeeze(0)
        # print(outt.shape)

        return self.dropout(outt) + identity
