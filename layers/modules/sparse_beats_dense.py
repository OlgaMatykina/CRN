...
class resnext_block(nn.Module):
    ...
    def forward(self, x):
        proj = x
        # print(f"x.shape: {x.shape}, proj.shape: {proj.shape}")
        if self.has_proj:
            if hasattr(self, "dsp"):
                proj = self.dsp(proj)
            # print(f"x.shape: {x.shape}, proj.shape: {proj.shape}")
...
class FullyConnected(torch.nn.Module):
    ...
    def forward(self, x):
        fully_connected = self.fully_connected(x.float())
...
class SparseBeatsDense(nn.Module):
    def __init__(self):
        super().__init__()
        self.G1  = UNet(4)        # 1/4
        self.G2  = UNet(12)       # 1/2      
    ...
    def forward(self, img, radar, radar_pts, valid_radar_pts_cnts):
        ...
        # print(img_dw2.shape, radar_dw2.shape)

        data_dw2 =  torch.cat((img_dw2,radar_dw2), dim=1)
        
        img_dw4 = nnf.interpolate(img, scale_factor=0.25, mode="bilinear")
        radar_dw4 = self.downsample_depthmap(radar, 0.25)
        data_dw4 =  torch.cat((img_dw4,radar_dw4),dim=1)
        ...
        data = torch.cat((img, radar, fm2), axis=1)
    ...
    def downsample_depthmap(self, inp, factor):
        ...
        nonzero_arr = nonzero_arr.to(out.dtype)
