...
from layers.modules.sparse_beats_dense import SparseBeatsDense
...
class DepthNet(nn.Module):
    ...
    def forward(self, x, mats_dict):
        ...
        if self.camera_aware:
            ...
            mlp_input = mlp_input.to(self.bn.weight.device)
...
class RVTLSSFPNSBD(BaseLSSFPN):
    def __init__(self, **kwargs):
        super(RVTLSSFPNSBD, self).__init__(**kwargs)
        ...
        self.sbd_net = self._configure_sbd_net()
    ...
    def _configure_sbd_net(self):
        return SparseBeatsDense()
        #     depth_net_conf['in_channels'],
        #     depth_net_conf['mid_channels'],
        #     self.output_channels,
        #     self.depth_channels,
    ...
    def get_geometry_collapsed(self, sensor2ego_mat, intrin_mat, ida_mat, bda_mat,
        ...
        # print(sensor2ego_mat.device)
        # print(torch.inverse(intrin_mat.cpu()).cuda().device)
        ...
        if bda_mat is not None:
            ...
            bda_mat = bda_mat.to(points.dtype)
    ...
    def _forward_sbd_net(self, img, radar, radar_pts, valid_radar_pts_cnts):
        return self.sbd_net(img, radar, radar_pts, valid_radar_pts_cnts)
    ...
    def _forward_single_sweep(self,
                              ...
                              pts_context,
                              pts_occupancy,
                              img, radar, radar_pts, valid_radar_pts_cnts,
                              return_depth=False):
        ...
        # print('OLD_DEPTH', depth_occupancy.shape)

        B,C, H, W = img.shape
        # print('img', img.shape)
        # img = sweep_imgs[:, :, 1, ...] # CAM_FRONT
        # img = img.squeeze(1) #num_sweeps=1, delete that dim
        # print('img', img.shape)
        pred_pyramid, valid_pyramid = self._forward_sbd_net(img, radar, radar_pts, valid_radar_pts_cnts)
        depth_occupancy = pred_pyramid[-1][:,:, :H,:W]

        depth_occupancy = torch.nn.functional.interpolate(depth_occupancy, size=(16, 44), mode='bilinear', align_corners=False)

        # print('NEW_DEPTH', depth_occupancy.shape)
