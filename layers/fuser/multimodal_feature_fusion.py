...
from ..modules.conf_guided_multimodal_cross_attention import DeformableCrossAttention
...
class RCFuser(nn.Module):
    def __init__(self, num_sweeps=4, img_dims=80, pts_dims=128, embed_dims=256,
        ...
        # self.input_proj_conf = nn.Linear(pts_dims, self.embed_dims)
        self.confmap_proj = nn.Sequential(
            nn.Linear(img_dims, self.embed_dims),
        ...
        self.confmap_conv = nn.Sequential(
            nn.Conv2d(embed_dims * num_sweeps,
                      embed_dims,
                      kernel_size=3,
                      stride=1,
    ...
    def _forward_single_sweep(self, feat_img, feat_pts):
        ...
        feat_img_to = feat_img.flatten(2).permute(0, 2, 1).contiguous()  # [bs, num_cam, c, dw] -> [num_cam, bs, dw, c]
        # print(feat_img_to.shape)
        feat_img_to = feat_img_to.reshape(bs,-1,80)# 32768,80
        confmap_kv = self.confmap_proj(feat_img_to)
        # C,B = confmap_kv.shape[:2]

        # print(feat_img_to.shape)
        confmap_kv = confmap_kv.reshape(bs, 16384 *self.embed_dims,1)
        confmap_kv = self.avgpool(confmap_kv).unsqueeze(-1)
        ...
        # confmap_kv = self.input_proj_conf(confmap_kv).softmax(-1)
